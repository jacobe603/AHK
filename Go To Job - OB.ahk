...
;Go to Order Book based on Window Title or number to left of cursor (CTRL + SHIFT + TILDE). This is only working matches in the Window Title and copy text method.
^+`::
WinGetTitle, wintitle, A
...
GoToJobFolder(match)     ; Function that sorts through folder names on S: drive to find the match
{
    count := ""
    Loop, Files, S:\Projects\%match%*, D
        {
