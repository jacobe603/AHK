...
#Include Library\JSON.AHK
    ...
    URL:="https://objm.svl.com/svl-services/rest/detail?orderNo=" . JobNumber
    oHTTP.Open("GET",URL)
    ...
    index := 1
        ; Create the ListView:
        Gui, Add, ListView, r20 w1000 gMyListView vMyListView altsubmit grid, Line No|Description|docTypeDesc|dtCost|postingDate
        for index, value in parsed.result
        {
            totalVol += value.lnVolume
            LV_Add("",value.lineNo,value.description,value.docTypeDesc,value.dtCost, SubStr(value.postingdate,1,10))
        }
